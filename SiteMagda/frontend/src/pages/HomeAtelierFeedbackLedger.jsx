...
const ALL_TESTIMONIALS = [FEATURED_TESTIMONIAL, ...MORE_TESTIMONIALS];

export default function HomeAtelierFeedbackLedger() {
  ...
  return (
    <div className="bg-paper">
      ...
      {/* Feedback — "Ledger List": every testimonial as one row, author in
          a mono column, hairline dividers between — reads like a ledger of
          endorsements rather than a quote or a card grid. */}
      <section className="bg-paper py-24 md:py-36 border-t border-ink/10">
        <div className="mx-auto max-w-5xl px-6 md:px-8">
          <Reveal className="text-center">
            <p className="font-mono text-xs tracking-[0.3em] text-gold-deep uppercase mb-4">Feedback</p>
            <h2 className="font-display text-3xl md:text-5xl leading-snug text-ink">
              Não confie em nós, confie em quem já confiou.
            </h2>
            <TickRule className="w-10 mx-auto mt-8" />
          </Reveal>

          <div className="mt-12 md:mt-16 divide-y divide-ink/10 border-t border-ink/10">
            {ALL_TESTIMONIALS.map((t, i) => (
              <Reveal
                key={t.author}
                delay={i * 60}
                className="grid grid-cols-1 md:grid-cols-[10rem_1fr] gap-3 md:gap-10 py-8 md:py-10"
              >
                <p className="font-mono text-xs tracking-widest uppercase text-gold-deep">{t.author}</p>
                <p className="font-display text-lg md:text-xl leading-relaxed text-ink">“{t.text}”</p>
              </Reveal>
            ))}
          </div>
        </div>
