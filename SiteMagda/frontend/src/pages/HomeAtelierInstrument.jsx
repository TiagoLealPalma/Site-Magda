...
// Atelier / "Instrument Rail" — the properties rail leans hardest into the
// blueprint identity: a mono position counter and a gold scrubber track (the
// exact device DESIGN.md already documents for the lightbox), slower pacing
// at two large cards per view, and a spec annotation line per card.
export default function HomeAtelierInstrument() {
  ...
  useEffect(() => {
    ...
    if (!el || !hasProperties) return;
    function onScroll() {
      const cardWidth = el.scrollWidth / properties.length;
      setActiveIndex(Math.min(properties.length - 1, Math.round(el.scrollLeft / cardWidth)));
    }
  ...
  return (
    <div className="bg-paper">
      ...
      {/* Featured properties — Instrument Rail */}
      <section className="bg-paper py-24 md:py-36 border-t border-ink/10">
        <div className="mx-auto max-w-7xl px-6 md:px-8">
          <Reveal className="mb-14 md:mb-16 flex flex-wrap items-end justify-between gap-6">
            ...
            <div className="flex items-center gap-6 shrink-0">
              {hasProperties && (
                <p className="font-mono text-xs tracking-widest text-stone">
                  {String(activeIndex + 1).padStart(2, "0")} / {String(properties.length).padStart(2, "0")}
                </p>
              )}
              <Link
                to="/imoveis"
                className="hidden md:inline-block border-b border-gold text-sm tracking-wide pb-1 hover:text-gold-deep transition-colors"
              >
                Ver todos →
              </Link>
            </div>
        ...
        {properties === null ? (
          <div className="pl-6 md:pl-8 flex gap-6 md:gap-10 overflow-x-hidden pb-4">
            {[0, 1].map((i) => (
              <div key={i} className="shrink-0 w-[85vw] sm:w-[70vw] lg:w-[46vw] aspect-[4/3] bg-ink/5" />
            ))}
          </div>
        ) : properties.length === 0 ? (
          <div className="mx-auto max-w-7xl px-6 md:px-8">
          ...
          <>
            <div className="relative">
              ...
              >
                {properties.map((p, i) => (
                  <div key={p.id} className="shrink-0 w-[85vw] sm:w-[70vw] lg:w-[46vw]">
                    <p className="mb-3 font-mono text-[11px] tracking-widest text-stone uppercase">
                      {String(i + 1).padStart(2, "0")} — {p.typology || (p.bedrooms ? `T${p.bedrooms}` : "—")} ·{" "}
                      {p.area} m² · {formatPrice(p.price)}
                    </p>
            ...
            {/* Gold scrubber track — the rail's own dimension line. */}
            <div className="mx-auto max-w-7xl px-6 md:px-8 mt-6">
              <div className="relative h-px bg-ink/10 max-w-xs">
                <div
                  className="absolute top-0 h-px bg-gold transition-[left] duration-150 ease-out"
                  style={{
                    width: `${100 / properties.length}%`,
                    left: `${(activeIndex / Math.max(1, properties.length - 1)) * (100 - 100 / properties.length)}%`,
                  }}
                />
              </div>
            </div>
